...
int main() {
  ...
  //Enhancement by adding a 3 pixel wide "window frame"
  ////////CANT RUN UNLESS PIXELATE IS COMMENTED OUT//////////////
  int window[MAX_H][MAX_W];
  for (int row = 0; row < h; row++){
    for (int col = 0; col < w; col++){
      // Check for the vertical middle, the top and bottom of image
      if (row < 3 || row > h-4 || (row > h/2-2 && row < h/2+2)){
        //Make these pixels white
        window[row][col] = 255;
      }
      else {
        // Check for the horizontal middle, the left, and right of the image
        if ((col > w/2-2 && col < w/2+2) || col < 3 || col > w-4){
          //Make these pixels white
          window[row][col] = 255;
        }
        else {
          //Make everything else the regular image
          window[row][col] = img[row][col];
        }
      }
    }
  }
  writeImage("window.pgm",window, h, w);
